...
githook_reconfigure_sandbox() {
    [ -z "$CSG_TOP" ] && echo "CSG_TOP not set. Git hook won't run." && return
    ...
    [ $call_sandbox_config -eq 0  -a  \( "$CSG_YARN" == "true"  -o  "$CSG_YARN" == "1" \) ] && githook_file_has_changed yarn.lock && call_sandbox_config=1
    [ $call_sandbox_config -eq 1 ] && echo "executing sandbox/configure-sandbox git-hook" && sandbox/configure-sandbox git-hook
}

[ "$CSG_DISABLE_GIT_HOOKS" == 1 ] && echo "hooks disabled (CSG_DISABLE_GIT_HOOKS=1)" && exit 0
